...
{
	int 			sockfd;
	struct sockaddr_in 	local_addr;	/* local socket 
						 * address for recv 
						 */
	struct sockaddr_in 	peer_addr;	/* peer(client) 
						 * socket address 
						 * raise the request
						 */
	socklen_t		peer_len;
	...
	mreq.imr_multiaddr.s_addr = inet_addr(IPADDR_GROUP);
	mreq.imr_interface.s_addr = 
		htonl(INADDR_ANY); //chosen by the system
	if (setsockopt(
			sockfd, 
			IPPROTO_IP,		/* level */
	...
	for (;;) {
		/* clean up buffer to receive data */
		...
		/* in case of overflow */
		if (rcvlen == MAXLINE) buf_rcv[MAXLINE-1] = 0x00;
